...
def _find_sum_to_value(value: int, numbers: List[int], num_addends: int, current_sum=0) -> Optional[List[int]]:
    if num_addends == 1:
        ...
        for number in range(len(numbers)):
            sums = _find_sum_to_value(value, numbers[number+1:], num_addends-1, current_sum + numbers[number])
            if sums:
